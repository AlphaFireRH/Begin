...
-(void)changeToInsets:(UIEdgeInsets)insets targetOrientation:(ScreenOrientation)orientation {
    ...
    //得到UIView 的缩放比率 (iphone7:2; iphonex:3, iPad4:2) >=-Rct-=<
    float edgeThinRate = unityView.contentScaleFactor;
    //把四条边按比率缩小
    UIEdgeInsets insetsReal = UIEdgeInsetsMake(insets.top/edgeThinRate, insets.left/edgeThinRate, insets.bottom/edgeThinRate, insets.right/edgeThinRate);
    
    CGRect viewRect = unityView.frame;
    //CGSize screenSize = [[[UIScreen mainScreen] bounds] size];
    CGSize screenSize = UIScreen.mainScreen.bounds.size;

    CGRect sizeUnityView =  CGRectMake(0, 0, screenSize.width, screenSize.height);

    
    if (orientation == landscapeLeft || orientation == landscapeRight) {
        if (BELOW_IOS_8) {
            viewRect = CGRectMake(viewRect.origin.x, viewRect.origin.y, viewRect.size.height, viewRect.size.width);
            self.toolBar.frame = CGRectMake(0, sizeUnityView.size.width - 44, sizeUnityView.size.height, 44);
            self.spinner.frame = CGRectMake(sizeUnityView.size.height / 2 - 65, sizeUnityView.size.width / 2 - 65, 130, 130);
        } else {
            self.toolBar.frame = CGRectMake(0, sizeUnityView.size.height - 44, sizeUnityView.size.width, 44);
            self.spinner.frame = CGRectMake(sizeUnityView.size.width / 2 - 65, sizeUnityView.size.height / 2 - 65, 130, 130);
        }
    } else {
        self.toolBar.frame = CGRectMake(0, sizeUnityView.size.height - 44, sizeUnityView.size.width, 44);
        self.spinner.frame = CGRectMake(sizeUnityView.size.width / 2 - 65, sizeUnityView.size.height / 2 - 65, 130, 130);
    ...
    CGRect f = CGRectMake(insetsReal.left,
                          insetsReal.top,
                          sizeUnityView.size.width - insetsReal.left - insetsReal.right,
                          sizeUnityView.size.height - insetsReal.top - insetsReal.bottom);
    self.frame = f;
    self.insets = insetsReal;
}
...
//增加一个简易的WebRequest 不使用NSMutableURLRequest, 不定制headers >=-Rct-=<
-(void) webviewName:(NSString *)name beginLoadURLPure:(NSString *)urlString{
    UniWebView *webView = [_webViewDic objectForKey:name];
...
extern "C" {
    ...
    void _UniWebViewLoadPure(const char *name, const char *url);    //>=-Rct-=<
...
//>=-Rct-=<
void _UniWebViewLoadPure(const char *name, const char *url) {
    [[UniWebViewManager sharedManager] webviewName:UniWebViewMakeNSString(name)
                                      beginLoadURLPure:UniWebViewMakeNSString(url)];
}
